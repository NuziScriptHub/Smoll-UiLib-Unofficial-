...
-- Theme
ThemeManager.Themes = { Red = Color3.fromRGB(255, 60, 60), Blue = Color3.fromRGB(60, 120, 255), Green = Color3.fromRGB(60, 255, 100), Yellow = Color3.fromRGB(255, 255, 60), Purple = Color3.fromRGB(160, 60, 255), Black = Color3.fromRGB(30, 30, 30), White = Color3.fromRGB(245, 245, 245), Gray = Color3.fromRGB(128, 128, 128), MTAnime = Color3.fromRGB(160, 60, 255) }
ThemeManager.Current = "Red"

function ThemeManager.ApplyTheme(themeName) local color = ThemeManager.Themes[themeName] or ThemeManager.Themes["Red"] ThemeManager.Current = themeName

for _, guiObject in pairs(game.CoreGui:GetDescendants()) do
    if guiObject:IsA("Frame") or guiObject:IsA("TextButton") or guiObject:IsA("TextLabel") then
        if guiObject.BackgroundColor3 == Color3.fromRGB(255, 60, 60) then
            guiObject.BackgroundColor3 = color
        end
        if guiObject.BorderColor3 == Color3.fromRGB(255, 60, 60) then
            guiObject.BorderColor3 = color
        end
    end
end

if themeName == "MTAnime" then
    for _, guiObject in pairs(game.CoreGui:GetChildren()) do
        if guiObject:IsA("ScreenGui") then
            local bg = Instance.new("ImageLabel")
            bg.Name = "MTAnimeBackground"
            bg.Parent = guiObject
            bg.Image = "rbxassetid://1234567890" 
    ...
    for _, guiObject in pairs(game.CoreGui:GetDescendants()) do
        if guiObject:IsA("ImageLabel") and guiObject.Name == "MTAnimeBackground" then
            guiObject:Destroy()
        end
    end
end
...
function nuhuh.CreateTextbox(conpig)
	conpig = conpig or {}
	conpig.Text = conpig.Text or "Enter text..."
	conpig.Placeholder = conpig.Placeholder or "Type here..."
	conpig.Callback = conpig.Callback or function(text) print("Text:", text) end
	...
	Frame.Size = UDim2.new(0, 370, 0, 20)

	Textbox.Name = "StyledTextbox"
	Textbox.Parent = Frame
	Textbox.BackgroundColor3 = Color3.fromRGB(225, 53, 53)
	Textbox.BorderSizePixel = 0
	Textbox.Position = UDim2.new(0.0027, 0, 0, 0)
	Textbox.Size = UDim2.new(0, 370, 0, 20)
	Textbox.Font = Enum.Font.SourceSans
	Textbox.PlaceholderText = tostring(conpig.Placeholder)
	Textbox.Text = ""
	...
	Textbox.FocusLost:Connect(function(enterPressed)
		if enterPressed then
			conpig.Callback(Textbox.Text)
		end
	end)
end


function nuhuh.CreateMultiTextbox(conpig)
	conpig = conpig or {}
	conpig.Text = conpig.Text or "Multiline Input"
	conpig.Placeholder = conpig.Placeholder or "Type here..."
	conpig.Callback = conpig.Callback or function(text) print("Text:", text) end
	...
	Frame.Name = "MultiTextbox"
	Frame.Parent = Main_2
	Frame.BackgroundColor3 = Color3.fromRGB(46, 46, 46)
	Frame.BackgroundTransparency = 1.000
	Frame.BorderSizePixel = 0
	Frame.Size = UDim2.new(0, 370, 0, 60)

	Label.Name = "MultiTextboxLabel"
	Label.Parent = Frame
	Label.BackgroundTransparency = 1.000
	Label.Size = UDim2.new(1, 0, 0, 15)
	Label.Font = Enum.Font.GothamMedium
	Label.Text = conpig.Text
	Label.TextColor3 = Color3.fromRGB(255, 255, 255)
	Label.TextSize = 12
	Label.TextXAlignment = Enum.TextXAlignment.Left

	Textbox.Name = "MultiTextboxInput"
	Textbox.Parent = Frame
	Textbox.BackgroundColor3 = Color3.fromRGB(225, 53, 53)
	Textbox.BorderSizePixel = 0
	...
	Textbox.Font = Enum.Font.SourceSans
	Textbox.PlaceholderText = conpig.Placeholder
	Textbox.Text = ""
...
function nuhuh.CreateKeyblind(conpig)
	conpig = conpig or {}
	conpig.Text = conpig.Text or "Keybind"
	conpig.Default = conpig.Default or Enum.KeyCode.RightShift
	conpig.Callback = conpig.Callback or function(key) print("Key pressed:", key.Name) end

	local Frame = Instance.new("Frame")
	local Label = Instance.new("TextLabel")
	local KeyButton = Instance.new("TextButton")
	local listening = false
	local currentKey = conpig.Default

	Frame.Name = "Keybind"
	Frame.Parent = Main_2
	Frame.BackgroundTransparency = 1.000
	Frame.Size = UDim2.new(0, 370, 0, 25)
	...
	KeyButton.BackgroundColor3 = Color3.fromRGB(225, 53, 53)
	KeyButton.Position = UDim2.new(0.6, 5, 0, 0)
...
function nuhuh.CreateColorPicker(conpig)
	conpig = conpig or {}
	conpig.Text = conpig.Text or "Pick a color"
	conpig.Default = conpig.Default or Color3.fromRGB(255, 60, 60)
	conpig.Callback = conpig.Callback or function(color) print("Color picked:", color) end
	...
	ColorButton.BackgroundColor3 = conpig.Default
	ColorButton.Position = UDim2.new(0.6, 5, 0, 0)
	ColorButton.Size = UDim2.new(0.35, 0, 1, 0)
	...
	ColorButton.MouseButton1Click:Connect(function()
		...
		conpig.Callback(newColor)
	end)
end

function nuhuh.CreateProgressBar(conpig)
	conpig = conpig or {}
	conpig.Text = conpig.Text or "Loading..."
	conpig.Progress = conpig.Progress or 0.5
	conpig.Color = conpig.Color or Color3.fromRGB(255, 60, 60)
	...
	Frame.Parent = Main_2
	Frame.BackgroundTransparency = 1.000
	Frame.Size = UDim2.new(0, 370, 0, 30)
	...
	BarBG.Name = "BarBG"
	BarBG.Parent = Frame
	BarBG.BackgroundColor3 = Color3.fromRGB(36, 36, 36)
	BarBG.Position = UDim2.new(0, 0, 0, 15)
	BarBG.Size = UDim2.new(1, 0, 0, 10)

	BarFill.Name = "BarFill"
	BarFill.Parent = BarBG
	BarFill.BackgroundColor3 = conpig.Color
	BarFill.Size = UDim2.new(math.clamp(conpig.Progress, 0, 1), 0, 1, 0)

	local prog = {}
	function prog.SetProgress(val)
...
function uh.DestroyUI()
	if createdGui and createdGui.Parent then
		createdGui:Destroy()
		createdGui = nil
	end
end
